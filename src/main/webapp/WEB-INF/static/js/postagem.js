$(document).ready(function () {
	...
	$( "#save-ajax" ).submit(function( event ) {
		  ...
		  $.post( '/wiki/postagem/ajax/save?_csrf=' + token, $(this).serialize() )
...
function search(value) {
	...
	if (autorId > 0) {
		url = "/wiki/postagem/ajax/autor/"+ autorId + "/titulo/" + value + "/page/1";
	} else {
	    url = "/wiki/postagem/ajax/titulo/" + value + "/page/1";
	}
...
function tbody(page) {
	...
	if (autorId > 0 && titulo.length > 0) {
		url = "/wiki/postagem/ajax/autor/"+ autorId + "/titulo/" + titulo + "/page/" + page;
	} else if (titulo.length > 0) {
		url = "/wiki/postagem/ajax/titulo/" + titulo + "/page/" + page;
	} else if (autorId > 0) {
		url = "/wiki/postagem/ajax/autor/" + autorId + "/page/" + page;
	} else {
		url = "/wiki/postagem/ajax/page/" + page;
	}
	
	$( "#table-ajax" ).load( url, function( response, status, xhr ) {
		
		  if ( status == "error" ) {
		    var msg = "Desculpa porém encontramos um erro: ";
		    $( "#info" ).html( msg + xhr.status + " " + xhr.statusText );
		  }
